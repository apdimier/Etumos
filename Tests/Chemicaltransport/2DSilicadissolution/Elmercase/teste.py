...
#
module.setCouplingParameter(initialTimeStep          = 500,
                            minTimeStep              = 5.0e+2,
                            maxTimeStep              = 1.e+4,
...
# possible values are CG CGS BiCGStab TFQMR GMRES
				       preconditioner='ILU2',accelerator="BiCGStab",thetaScheme=0.0)
module.setVtkOutputsParameters(["Na_output","pH_output"],"d",8)
...
mass_soude = 5.E-5
print '============================================= '
print 'Calcul d errors pour le pH '
print '============================================= '
...
for i in res_ph[-1][1].values[0]:
	...
	#print "delt ",delt
        error=max(error,delt)
        pass
            #pass
    if (delt) > epsilon:
        OK = 0
	inderr+=1
        print 'diff too high for pH:',ind,a,b,numerical_value,analytical_value,error
    ind+=1
print "inderr ",inderr
print " max of error ",error,error*100
ind = 0
for i in res_ph[-1][1].values[0]:
    a = res_ph[-1][1].values[0][ind]
    b = res_ph[-1][1].values[1][ind]
...
print 'maximal error for pH = ', error," and % of error is ",error*100
OKConc = 1
if (OK!=0):
    print "\n \n -- Test-case Alkaline 2D with ELMER/PHREEQC coupling OK fo pH-- \n \n"
    pass
#else:
...
for i in res_na[-1][1].values[0]:
    ...
    delt = abs(analytical_value-numerical_value)
    error=max(error,delt)    
    if difference=='rel' and abs(analytical_value)>1.e-05:
        delt/=analytical_value
        error=max(error,delt)
        pass
    if (delt) > epsilon:
        OK = 0
	inderr+=1
        print 'diff too high for Na:',ind,a,b,numerical_value,analytical_value
        pass
    ind+=1
print "inderr ",inderr
ind = 0
for i in res_na[-1][1].values[0]:
    ...
    #print 'numerical value :',a,b,numerical_value
print " maximal concentration ", imaxcont,jmaxcont, maxcont  
print 'maximal error for Na = ', error," and % of error is ",error*100
OKConc = 1
if (OK):
