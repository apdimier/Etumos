#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#  Analytical solution for a 2D diffusion/dispersion problem
#
...
from typechecktools import verifyClass, verifyItem

class AnalyticalFunction_pH_62_2D(Function):
    def __init__(self, time, masse):
        self.time  = time
        self.masse = masse
        pass
...
#        c = (self.masse)/(4.0*pi*Ux*pow(alphaL*alphaT,0.5))
        c = (self.masse)/(4.0*pi*Ux*self.time*pow(alphaL*alphaT,0.5))
        res1 = c*(exp(-1.*(a+b)))
