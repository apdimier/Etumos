...
try:
    ...
    try:
        ...
        try:
            ...
            numpy Python site (http://numpy.scipy.org) for information on
...
#                    else:                                        
        ...
        if unstructured :
            ...
            bodyPorosities =  NumericBodyField('porosity', self.mesh,['poro'], numpy.float)
            
            if self.temperature:
                thermalConductivityZone = InstanceBodyField('thermalconductivity',  self.mesh, type = Tensor)
                self.thermalDelayedFactor_zone = NumericBodyField('thermaldelayedfactor',  self.mesh, ['phirt'], numpy.float)
                pass
            print "\neffective_diffusion:\n";#sys.stdout.flush()
            
            effective_diffusion_zone = InstanceBodyField('diffusion', self.mesh,type = Tensor)
            
            dispersivityBody = NumericBodyField('dispersivity', self.mesh,['alphal','alphat'], numpy.float)
...
#                                                ( self.fixedCk[ind] - self.fixedCn[ind] )
                ...
                print "ctmdbg self.variablePorosityOption",self.currentPorosity[0],self.currentPorosity[1],self.currentPorosity[2]
            ...
            for boundary in self.boundaryConditions:
                ...
                if self.temperature:
                    self.dirichletBodyCond = NumericBodyField('Dirichlet boundary condition ',
                                                              self.mesh, self.componentList+['temperature'], numpy.float)
                else:
                    self.dirichletBodyCond = NumericBodyField('Dirichlet boundary condition ',
                                                              self.mesh, self.componentList, numpy.float)
                #
