...
columnMaterial = Material(name="Mat",hydraulicconductivity = HydraulicConductivity(value = K),\
                          ...
                          saturationLaw = VanGenuchtenSaturation(alpha,n,m))
...
print velocity[0][0],velocity[0][1]
#raw_input()
module.writeVelocityPlot()
eps = 1.e-3
ok = 1
for v in velocity:
    norml2 = ((velocity[0][0]+7.4843677755)**2+(velocity[0][1]-0.)**2)**0.5
    if norml2> eps:
        raise Exception, " The test on the RichardsDyke failed",norml2
if ok:
    print "\n \n ~~~~~~~~~~~~~~~~~~~ The test case on the RichardsDyke is ok ~~~~~~~~~~~~~~~\n"
    pass
else:
