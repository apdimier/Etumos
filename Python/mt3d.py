...
from generictools import c_archi, isInstance, Generic

if c_archi()=='32':
    from numpy import array, float32 as Float
    pass
else:
...
class Mt3d(Generic):
    ...
    def getCoordinatesValues(self):
        ...
        if self.coordinates == None:
            if (len(self.delr)!=1 and len(self.delc)!=1):
                ...
                raise Warning, "getCoordinatesValues problem"
            pass
        else:
            pass
    ...
    def init(self, name = None):
        ...
        if name:
            ...
            raise Warning, " No name defines by the user"
            pass
    ...
    def setAdditionalSourceValues(self,source):
        """
        Transfer of a concentration field to Mt3d, 
        which corresponds to the source term necessary to the Picard algorithm.
        """
...
#
        ...
        self.basFile.write("%10i%10i%10i%10i%10i   !nlay,nrow,ncol,nper,itmuni\n"\
                            %(nlay,self.mesh.getNbPoints("Y")-1,self.mesh.getNbPoints("X")-1,1,1))
...
#        self.basFile.write("%15s%10s%10s   !perlgn, nstp, tsmult"%(_fote150(3.e+4),_foti10(1),_fote105(20.0)))
    ...
    #
        ...
        self.file_sip.write("%10s%10s%10d%10s%10d !ACCL WCLOSE IPCALC WSEED IPRSIP\n"\
                            %(_fote104(0.5),_fote104(1.e-20),1,_fote104(1.e-17),100))
        self.file_sip.close()
...
#       os.remove('IOUTF')
    ...
    def setGradient(self,scheme,residual):
        ...
        """
        if scheme.upper()=="JACOBI":
            self.isolve = 1
            pass
        elif scheme.upper()=="SSOR":
            self.isolve = 2
            pass
        elif scheme.upper()=="MIC":
            self.isolve = 3
            pass
        else:
...
#        list_regionc = self.getRegions()
        ...
        for nom_region in list_regionc:
            ...
            j1 = ind_min.j
            j2 = ind_max.j
            #print " dbg mt3d i1 i2 j1 j2 ",i1,i2,j1,j2
            for i in range(i1,i2+1,1):
                for j in range(j1,j2+1,1):
                    k = (j-1)*ncol+i
        ...
        for nom_region in list_regionc:
            ...
            j1 = ind_min.j
            j2 = ind_max.j
            #print " dbg porosity_region ",porosity_region,i1,i2,j1,j2
            for i in range(i1,i2+1,1):
                for j in range(j1,j2+1,1):
                    k = (j-1)*ncol+i
...
#
        ...
        self.dspFile.write("\n") 
        self.dspFile.close()
        #print " dbg mt3d end of the dsp file"
        #
...
#        file = self.monodadv
    ...
    def kontrolle(a,argument,comment = None):
        if type(a).__name__ != argument:
            ...
            raise Warning, message+str(type(a).__name__)
