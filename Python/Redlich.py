...
fo a real gas, we have to take components interactions into account. Therefore, we introduce a correction in the preceding equation: 
µ(T) = µ0 (T) + RT ln (p/p0 )+ RT ln (phi)
with phi the fugacity coefficient; leading to :

µ(T) = µ0 (T) + RT ln (f/p0 )

f=phi*p is the gas fugacity.
...
def co2FugacityEva(V,R,temperature,pressure,aco2,bco2):
    """
...
def co2Fugacity(pressure, temperature):
    """
    See the co2FugacityEva function. here the function has less arguments   
    """
    ...
    # molar volume
    #
    V = polyCubicRoots(a2, a1, a0)
    #
    #
    cte = R*(temperature**1.5)*bco2
    phico2 = log(V/(V - bco2)) + (bco2/(V - bco2)) - \
        (2.*(1.*aco2)/( cte ) ) * log((V + bco2)/V) + \
        (aco2*bco2/(cte*bco2))*(log((V + bco2)/V) - (bco2/(V + bco2)) ) - log(pressure*V/(R*temperature))
...
def K0CO2g(temperature):
    Tc = temperature - 273.15
    return exp(log(10)*(1.189 + (1.304e-2 - 5.446e-5*Tc)*Tc))
...
def co2Solubility(saltAmount,pressure,temperature):
    ...
    #
    print " value of V bco2,aco2 ",vco2, bco2, aco2, R, cte, pressure, temperature

    co2FugacityCoef = co2FugacityEva(vco2,R,temperature,pressure,aco2,bco2)
    ...
    if mNa > 0:
        #
        # The summation convention of einstein is not taken into account in the articles ( it seems that..)
        #
...
#
def virialFugacityCoefficient(pressure, temperature, gasName):
    """

        The fugacity coefficient is calculated here from the eq.(14) 
        of the paper of Nicolas Spycher et al., 1988:

        Spycher, N.F. and Reed, M.H. 1988 
        Fugacity coefficients of H2, CO2, CH4 and J2O-CO2-CH4 mixtures: a virial equation treatment for moderate pressures and temperatures
        applicable to hydrothermal boiling.
        Geochim. Cosmochim Acta, 52, 739-749

        the validity range is  16-350 0C and 0-500 bar

        The temperature is entered in K, the pressure in bars
    """
    invTemp = 1./temperature
    if gasName.lower() == "co2":          # Parameters for CO2 gas
        a =-1430.87e+0
        b = 3.59800e+0
        c =-227.376e-5
        d = 347.644e-2
        e =-1042.47e-5
        f = 846.271e-8
    elif gasName.lower() == "ch4":          # Parameters for CH4 gas
        a =-537.779e+0          
        b = 1.54946e+0
        c =-92.7827e-5
        d = 120.861e-2
        e =-370.814e-5
        f = 333.804e-8
    elif gasName.lower() == "h2":          # Parameters for H2 gas
        a =-12.5908e+0
        b = 0.259789e+0
        c =-7.24730e-5
        d = 0.471947e-2
        e =-2.69962e-5
        f = 2.15622e-8
    elif gasName.lower() == "h2s":          # Parameters for H2S gas (are the same as for CO2)
        a =-1430.87e+0             
        b = 3.59800e+0                   
        c =-227.376e-5                 
        d = 347.644e-2                 
        e =-1042.47e-5                 
        f = 846.271e-8                 
    elif gasName.lower() == "so2":          # Parameters for SO2 gas
        raise Warning, " no coefficient has been found for so2   "           
    return exp(( ( (a + 0.5*d*pressure)*invTemp + b + 0.5*e*pressure)*invTemp + c + 0.5*f*pressure )*pressure)          
