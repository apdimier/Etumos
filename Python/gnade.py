...
#-----------------------------------------------------------------------------
#
# Here the copyright has to be introduced
#
# Name:        gnade.py
# Purpose:     Interactive Graphs
#
# Author:      Dimier Alain
#
# Created:     2008/06/05
...
#
# Created:     2003/11/03
# RCS-ID:      $Id: gnade.py,v 1.2 2011/03/15 21:48:36 dimier Exp $
# Copyright:   (c) 2002
# Licence:     Use as you wish.
#-----------------------------------------------------------------------------
...
class PolyPoints:
    ...
    def getLegend(self):
        print(" polyline legend ",self.attributes['legend'])
...
class PlotGraphics:
    ...
    def __init__(self, objects, title='toto', xLabel='', yLabel= ''):
    ...
    def getSymExtent(self, printerScale):
        ...
        for obj in self.objects[1:]:
            oSymExt = obj.getSymExtent(printerScale)
...
class PlotCanvas(wx.Panel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition,
                 size=wx.DefaultSize, style=0, name="Gnade"):
    ...
    def SetFontSizeLegend(self, point= 10):
        """Set Legend font size (default is 10 point)"""
...
def _draw7Objects():
    ...
    print(" within _draw7Objects ",type(x))
    y1 = 4.5*x**2
    y2 = 2.2*x**3
...
def _drawNewObjects(xLabel = None, yLabel = None):
    # Empty graph with axis defined but no points/lines
    ...
    if yLabel=="None":
        yLabel = "Value Y"
    x = _Numeric.arange(1,10,1)
    y0 = 1+4.5*x
    y1 = 1+x + x**2
    y2 = 1+x + x**2+x**3
    points0 = _Numeric.transpose([x,y0])
    points1 = _Numeric.transpose([x,y1])
    points2 = _Numeric.transpose([x,y2])
    line0 = PolyLine(points0, legend='linear',colour = "blue", width=1)
    line1 = PolyLine(points1, legend='quadratic',colour = "red", width=1)
    line2 = PolyLine(points2, legend='cubic',colour = "green", width=1)
    return PlotGraphics([line0,line1,line2], "double log plot", xLabel, yLabel)
...
##        # profile end
    ...
    def OnPlotDraw8(self, event):
        ...
        self.resetDefaults()
        self.client.setLogScale((True,True))
        self.client.Draw(_drawNewObjects(xLabel = "x label",yLabel = "y label"))

    def OnPlotDraw9(self):
        #log scale example
        self.resetDefaults()
        self.client.setLogScale((True,True))
        self.client.Draw(_drawNewObjects(xLabel = "x label",yLabel = "y label"))
