...
class AquiferPropertyScalar(AquiferProperty):
    ...
    def checkValue(self, value):
        ...
        if not valueOK:
            try:
                memberShip(value, [Field, LinearFunction,TimeTabulatedFunction,SpaceAndTimeTabulatedFunction,
            ...
            pass
        if not valueOK:
            raise "physical quantity can\"t be defined, see the value %s"%(value)
        self.value = value
        return
...
class SpecificStorage(AquiferPropertyScalar):
    """
    ...
    def __init__(self, value=None, unit=None):
        ...
        AquiferPropertyScalar.__init__(self, value,unit=default_unit)
        pass
    pass
    ...
    def __init__(self, value = None, dxx = None, dyy = None, dzz = None, unit = unit):
        ...
        if value == None:
            ...
            if type(dzz) != NoneType:
                raise TypeError, "dzz type error ",dzz
            pass
        try:
            vol = val
        except:
...
class KinematicDispersion(Scalar):
    ...
    def __init__(self, longitudinal, transverse, unit=unit):
...
class ThermalConductivity(Tensor):
    ...
    def __init__(self,value = None,dxx = None,dyy = None,dzz = None, unit = "kg*m/s***3/K"):    
        ...
        if (dxx or dyy or dzz) and not value:
            if (dyy or dzz) and type(dxx)==NoneType:
                raise "of an effective diffusion"
            val=dxx
